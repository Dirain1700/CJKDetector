...
function getBaseTSConfigFile(tsconfig: tsconfigOptions): tsconfigOptions {
    if (tsconfig.extends) {
        ...
        if (baseTSConfig.extends)
            return getBaseTSConfigFile(merge(baseTSConfig, tsconfig, { extends: baseTSConfig.extends }));
    ...
    allDiagnostics.reduce((previousDiagnosticFileName: string, diagnostic) => {
        if (diagnostic.file) {
            ...
            if (areAllErrorsOnSameFile) {
                if (previousDiagnosticFileName && previousDiagnosticFileName !== diagnostic.file.fileName)
                    areAllErrorsOnSameFile = false;
            }
